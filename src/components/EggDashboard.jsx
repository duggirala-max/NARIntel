  ...
  useEffect(() => {
    ...
    workerRef.current.onmessage = async (e) => {
      ...
      if (status === 'SUCCESS') {
        ...
        const currentPrice = dashboardData?.namakkal || '0';
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.7rem', color: 'var(--nar-orange)', fontWeight: 'bold', margin: 0, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Egg Executive Intelligence (HS 0407)
            </p>
          </div>
        </div>
      ...
      <main style={{ padding: '3rem', flex: 1 }}>
        {!isDashboardActive ? (
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '50vh', textAlign: 'center' }}>
            <div style={{ width: '4px', height: '100px', backgroundColor: 'var(--nar-orange)', marginBottom: '2rem' }} />
            <h2 style={{ fontSize: '1.8rem', marginBottom: '1rem', fontWeight: '500' }}>
              Executive Surveillance Ready
            </h2>
            <p style={{ color: '#666', maxWidth: '500px', lineHeight: '1.8', fontSize: '0.95rem' }}>
              Select <strong>"Start Dashboard Monitoring"</strong> to synchronize with the latest market indicators and risk alerts.
            </p>
          </div>
        ) : isRunning ? (
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '50vh' }}>
            <div className="spinner" />
            <p style={{ marginTop: '1.5rem', color: 'var(--nar-orange)', fontWeight: 'bold', textTransform: 'uppercase', fontSize: '0.8rem' }}>
              Compiling External Field Reports...
            </p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '2rem' }}>
            ...
            <div style={{ gridColumn: 'span 8' }}>
              <h3 style={{ marginBottom: '1.5rem', fontSize: '0.9rem', textTransform: 'uppercase', color: '#888', letterSpacing: '0.05em' }}>
                <span style={{ color: 'var(--nar-orange)', marginRight: '8px' }}>◆</span>
                Global Risk Intelligence (HS 0407)
              </h3>
            ...
            <div style={{ gridColumn: 'span 4', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              ...
              <div style={{ backgroundColor: 'var(--nar-black)', color: 'white', padding: '2.5rem', borderRadius: '28px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
                <div style={{ fontSize: '0.72rem', color: '#888', textTransform: 'uppercase', marginBottom: '0.5rem', fontWeight: '600' }}>
                  Live Market Index (Namakkal)
                </div>
                <div style={{ fontSize: '2.8rem', fontWeight: 'bold', color: 'var(--nar-emerald)', letterSpacing: '-0.02em' }}>
                  INR {dashboardData?.namakkal || '0.00'}
                </div>
                <div style={{ fontSize: '0.75rem', color: '#aaa', marginTop: '1rem', lineHeight: '1.5' }}>
                  Synchronized with NECC regional poultry benchmarks.
                </div>
                <button
              ...
              <div style={{ border: '1px solid #eee', padding: '2.5rem', borderRadius: '28px', backgroundColor: 'white', boxShadow: '0 10px 30px rgba(0,0,0,0.02)' }}>
                ...
                {dataIntelligence ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.6rem' }}>
                    ...
                    <div style={{ borderLeft: '4px solid var(--nar-orange)', paddingLeft: '1.2rem' }}>
